...
- [x] Централизованная валидация входных данных (длина, тип, формат) // Частично
...
### 8. Конфигурация
- [ ] Валидация env-переменных при старте (с понятными ошибками)
- [ ] Разделение конфигурации на development/production/test
- [ ] Вынести магические числа (MAX_ATTEMPTS, BLOCK_WINDOW_MS) в конфиг
...
### 9. Кеширование
- [ ] Кеширование статических файлов (Cache-Control, ETag headers)
- [ ] Кеширование собранного HTML (сейчас пересобирается на каждый запрос)
- [ ] Кеширование DB-запросов для частых операций (список задач, баланс)
...
- [ ] Пагинация для history и requests (сейчас загружается всё)
- [ ] WebSocket для real-time обновлений (вместо pull-to-refresh)

### 12. База данных
- [ ] Connection pooling tuning (min/max/idle timeout)
- [ ] Добавить `updated_at` на все основные таблицы
- [ ] Миграция: автоматический rollback при ошибке
- [ ] Добавить `EXPLAIN ANALYZE` для медленных запросов

### 13. Мониторинг
- [ ] Health endpoint (`/api/health`) с проверкой БД и services
