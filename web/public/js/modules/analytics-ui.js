...
const SHOP_PROGRESS_GOAL = analyticsConfig.SHOP_GOAL || 300;
...
const WEEKLY_EARN_GOAL = analyticsConfig.WEEKLY_GOAL || 220;
const XP_PER_LEVEL = analyticsConfig.XP_PER_LEVEL || 120;

function clampPercent(value) {
    return Math.max(0, Math.min(100, Math.round(value)));
}

export function computeMiniAnalytics({ summary = {}, topItems = [], trends = [], recommendations = [] } = {}) {
    const totalEarned = Number(summary.totalEarned || 0);
    const topSpend = topItems.reduce((max, item) => Math.max(max, Number(item.coins || 0)), 0);
    const activeDays = trends.reduce((count, trend) => count + ((trend.earned || 0) > 0 ? 1 : 0), 0);
    const primaryRecommendation = recommendations[0]?.name || 'Нет рекомендаций';

    const dayProgress = clampPercent((totalEarned / DAILY_PROGRESS_GOAL) * 100);
    const shopProgress = clampPercent((topSpend / SHOP_PROGRESS_GOAL) * 100);
    const streakProgress = clampPercent((activeDays / 7) * 100);

    return {
        dayProgress,
        shopProgress,
        streakProgress,
        dayLabel: `${dayProgress}%`,
        shopLabel: `${shopProgress}%`,
        streakLabel: `${activeDays} дн.`,
        shopHint: `Рекомендуем: ${primaryRecommendation}`
    };
}
...
function getCurrentBalance() {
    ...
    return state.children.find(c => c.id == state.currentChildId)?.balance || 0;
}

function setSummaryAmount(id, value, iconHtml) {
    const element = document.getElementById(id);
    if (element) {
        element.innerHTML = `${value}${iconHtml}`;
    }
}

function updateNetBalance(currentBalance, iconHtml) {
...
function updateSummaryUI(summary) {
    ...
    setSummaryAmount('stats-earned', effectiveTotalEarned, iconHtml);
    setSummaryAmount('stats-spent', summary.totalSpent, iconHtml);
    setSummaryAmount('tasks-total-coins', `Всего: ${effectiveTotalEarned}`, iconHtml);
    setSummaryAmount('items-total-coins', `Всего: ${summary.totalSpent}`, iconHtml);
    updateNetBalance(currentBalance, iconHtml);
}
