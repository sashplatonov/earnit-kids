/** @file Blog controller — serves markdown articles from web/data/blog/ */
'use strict';
...
async function readPosts() {
    if (blogCache.posts && Date.now() - blogCache.fetchedAt < CACHE_TTL_MS) return blogCache.posts;
...
async function serveBlogIndex(req, res) {
    ...
    await renderView({ viewName: 'blog-index.html', replacements, headers: getHtmlHeaders(req), req, res });
}
...
async function serveArticle(req, res, slug) {
    ...
    await renderView({ viewName: 'article.html', replacements, headers: getHtmlHeaders(req), req, res });
}
