...
class PushResourceTest {
    ...
    void registerRequiresAuth() {
        ...
        Response ok = resource.register(contextWithAuth(new AuthContext("fam-1", 10, "child", "c@test.com", "csrf")));
    ...
    void unregisterRequiresAuth() {
        ...
        Response ok = resource.unregister(contextWithAuth(new AuthContext("fam-1", null, "admin", "a@test.com", "csrf")));
        assertThat(ok.getStatus()).isEqualTo(200);
    }
