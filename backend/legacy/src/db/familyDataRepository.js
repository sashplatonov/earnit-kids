...
async function fetchFamilyDataRaw({ dbId, childId, p, hw, rw }) {
    const executor = typeof getClient === 'function' ? await getClient() : null;
    const runQuery = executor ? executor.query.bind(executor) : query;
    ...
    try {
        ...
        return await Promise.all([
            runQuery(`SELECT t.*, t.group_name FROM tasks t WHERE t.family_id = $1 AND t.is_deleted = false${tasksFilter}`, taskParams),
            runQuery(`SELECT s.*, s.group_name FROM shop_items s WHERE s.family_id = $1 AND s.is_deleted = false${shopFilter}`, shopParams),
            runQuery(`SELECT h.* FROM history h ${hw} ORDER BY h.created_at DESC LIMIT 50`, p),
            runQuery(`SELECT r.*, t.name as task_name, i.name as item_name, t.group_name as task_group, i.group_name as item_group, t.comment as task_comment 
                FROM requests r 
                LEFT JOIN tasks t ON r.task_id = t.task_id 
                LEFT JOIN shop_items i ON r.item_id = i.item_id 
                ${rw} 
                ORDER BY r.created_at DESC`, p),
            childId ? runQuery('SELECT balance FROM children WHERE id = $1', [childId]) : { rows: [] }
        ]);
    } finally {
        if (executor) {
            executor.release();
