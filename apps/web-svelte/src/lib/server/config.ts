...
const DEFAULT_LEGACY_WEB_ORIGIN = 'http://localhost:3000';
...
export function loadAppConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
    return {
        backendOrigin: trimTrailingSlashes(env.BACKEND_ORIGIN || DEFAULT_BACKEND_ORIGIN),
        sessionPath: env.SESSION_PATH || DEFAULT_SESSION_PATH,
        wsPath: env.WS_PATH || DEFAULT_WS_PATH,
        legacyWebOrigin: trimTrailingSlashes(env.LEGACY_WEB_ORIGIN || DEFAULT_LEGACY_WEB_ORIGIN),
