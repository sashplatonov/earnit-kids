...
UPDATE history
SET reason = CASE WHEN type = 'spend' THEN 'REWARD_PURCHASE' ELSE 'TASK_REWARD' END
WHERE reason IS NULL;

UPDATE history
SET delta = CASE WHEN type = 'spend' THEN -ABS(amount) ELSE ABS(amount) END
WHERE delta IS NULL;
...
ALTER TABLE history ADD CONSTRAINT fk_history_reversal
    FOREIGN KEY (reverses_entry_id) REFERENCES history(id);
ALTER TABLE history ADD CONSTRAINT uq_history_reversal UNIQUE (reverses_entry_id);
ALTER TABLE history ADD CONSTRAINT ck_history_delta_amount
    CHECK (amount >= 0 AND delta <> 0);
