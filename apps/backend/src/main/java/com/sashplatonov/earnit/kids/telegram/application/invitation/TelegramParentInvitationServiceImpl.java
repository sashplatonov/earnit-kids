...
public class TelegramParentInvitationServiceImpl implements TelegramParentInvitationService {
  ...
  @Override
  @Transactional
  public OperationResult<TelegramLinkLaunchResponse> invite(
    ...
    invitations.persist(
        TelegramParentInvitationEntity.builder()
  ...
  @Override
  @Transactional
  public OperationResult<TelegramIdentityService.TelegramIdentity> accept(
      String token, String initData, Instant now) {
    var verified = verifier.verify(initData).orElse(null);
    if (verified == null) {
      return ServiceResults.failure(
          "TELEGRAM_INVITE_INVALID", "Telegram invitation is invalid or expired.");
    }
    ...
    ParentAccountEntity parent =
        existing.orElseGet(
            () -> {
              var created = ParentAccountEntity.builder().email(null).passwordHash("").build();
    ...
    var identity =
        identityService.linkParent(
            invitation.getFamilyId(),
            verified.telegramUserId(),
            parent.getId(),
            invitation.getParentName(),
            verified.telegramUsername(),
            verified.telegramDisplayName(),
            now);
  ...
  private OperationResult<TelegramIdentityService.TelegramIdentity> acceptLegacy(
    ...
    var verified = verifier.verify(initData).orElse(null);
    if (verified == null
        || identityService.findActiveByTelegramUserId(verified.telegramUserId()).isPresent()) {
      return ServiceResults.failure(
          "TELEGRAM_INVITE_INVALID", "Telegram invitation is invalid or expired.");
    }
    String secret = stripPrefix(token);
    var invitation =
        secret.isEmpty() ? null : invitations.findByDigestForUpdate(digest(secret)).orElse(null);
    if (invitation == null
        || invitation.getRevokedAt() != null
        || invitation.getConsumedAt() != null
        || !invitation.getExpiresAt().isAfter(now)) {
    ...
    var parent =
        existing.orElseGet(
            () -> {
              var created =
                  ParentAccountEntity.builder().email(normalized).passwordHash("").build();
              parents.persist(created);
              return created;
            });
    memberships.persist(
  ...
  private String stripPrefix(String token) {
    ...
    return token != null && token.startsWith(prefix) ? token.substring(prefix.length()) : "";
  }
