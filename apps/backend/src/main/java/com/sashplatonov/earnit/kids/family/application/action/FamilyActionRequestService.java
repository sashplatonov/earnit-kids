...
final class FamilyActionRequestService {
  ...
  FamilyActionRequestService(
      FamilyActionSupportService supportService,
  ...
  OperationResult<FamilyDataResponse> requestTaskCompletion(
      String familyId, int childId, long taskId, String note) {
    ...
    String taskLimitError =
        frequencyService.validateTaskRequestLimit(familyDbId, childId, task.get());
    ...
    String normalizedNote =
        normalizedNoteResult instanceof OperationResult.Success<String> success
            ? success.value()
            : null;

    PurchaseRequestEntity request =
        buildTaskRequest(familyDbId, childId, task.get(), normalizedNote);
  ...
  OperationResult<FamilyDataResponse> requestItemPurchase(
      String familyId, int childId, long itemId, String note) {
    OperationResult<Integer> familyDbIdResult = supportService.requireFamilyDbId(familyId);
    if (familyDbIdResult.isFailure()) {
      return familyDbIdResult.asFailure();
    }
    ...
    String itemLimitError =
        frequencyService.validateItemRequestLimit(familyDbId, childId, item.get());
    ...
    PurchaseRequestEntity request =
        buildPurchaseRequest(familyDbId, childId, item.get(), normalizedNote);
    purchaseRequestRepository.persist(request);
    purchaseRequestRepository.flush();
  ...
  OperationResult<FamilyDataResponse> approveRequest(
      String familyId, Integer currentChildId, long requestId) {
    OperationResult<Integer> familyDbIdResult = supportService.requireFamilyDbId(familyId);
    if (familyDbIdResult.isFailure()) {
      return familyDbIdResult.asFailure();
    }
    ...
    Optional<PurchaseRequestEntity> request =
        supportService.findFamilyRequestForUpdate(familyDbId, requestId);
    if (request.isEmpty()) {
    ...
    Optional<ChildEntity> child =
        supportService.findFamilyChildForUpdate(familyDbId, request.get().getChildId());
    if (child.isEmpty()) {
      return OperationResult.failure(BackendMessages.message("family.childNotFound"));
    }
    if (supportService.isInactive(child.get())) {
    ...
    Optional<ShopItemEntity> item =
        request.get().getItemId() == null
            ? Optional.empty()
            : supportService.findActiveItem(
                familyDbId, request.get().getChildId(), request.get().getItemId());
    Optional<TaskEntity> task =
        request.get().getTaskId() == null
            ? Optional.empty()
            : supportService.findActiveTask(
                familyDbId, request.get().getChildId(), request.get().getTaskId());

    OperationResult<Void> balanceResult = applyApprovalBalance(familyDbId, request.get(), child.get());
    if (balanceResult.isFailure()) {
      return balanceResult.asFailure();
    }

    historyRepository.persist(
        historyFactory.buildRequestHistory(familyDbId, request.get(), item, task));
    request.get().setStatus(PurchaseRequestStatus.approved);
    publish(
        FamilyActionRequestSupport.isPurchase(request.get())
            ? ApplicationOutboxEventType.REWARD_APPROVED
    ...
    int responseChildId =
        supportService.resolveResponseChildId(
            familyDbId, currentChildId, request.get().getChildId());
    return supportService.loadFamilyData(familyId, responseChildId, true);
  }

  private OperationResult<Void> applyApprovalBalance(
      int familyDbId, PurchaseRequestEntity request, ChildEntity child) {
    if (!FamilyActionRequestSupport.isPurchase(request)) {
      child.setBalance(child.getBalance() + request.getCoins());
      return OperationResult.success(null);
    }
    long rewardLimit = child.getDailyRewardLimit();
    if (rewardLimit > 0
        && supportService.dailyRewardSpend(
                request.getChildId(), supportService.startOfFamilyDay(familyDbId, historyFactory.now()))
            + request.getCoins()
            > rewardLimit) {
      return OperationResult.failure(BackendMessages.message("balance.rewardLimitReached"));
    }
    if (child.getBalance() < request.getCoins()) {
  ...
  OperationResult<FamilyDataResponse> rejectRequest(
      String familyId, Integer currentChildId, long requestId) {
    OperationResult<Integer> familyDbIdResult = supportService.requireFamilyDbId(familyId);
    if (familyDbIdResult.isFailure()) {
      return familyDbIdResult.asFailure();
    ...
    publish(
        FamilyActionRequestSupport.isPurchase(request.get())
            ? ApplicationOutboxEventType.REWARD_REJECTED
            : ApplicationOutboxEventType.TASK_REJECTED,
        request.get(),
        0,
        null);
  ...
  OperationResult<FamilyDataResponse> deleteRequest(
      String familyId, Integer currentChildId, long requestId) {
    OperationResult<Integer> familyDbIdResult = supportService.requireFamilyDbId(familyId);
    if (familyDbIdResult.isFailure()) {
      return familyDbIdResult.asFailure();
    ...
    Optional<PurchaseRequestEntity> request =
        supportService.findFamilyRequest(familyDbId, requestId);
    if (request.isEmpty()) {
      return OperationResult.failure(BackendMessages.message("requests.notFound"));
    }

    boolean isChildDeletingOwnRequest =
        currentChildId != null && Objects.equals(request.get().getChildId(), currentChildId);
  ...
  private PurchaseRequestEntity buildTaskRequest(
      int familyDbId, int childId, TaskEntity task, String note) {
  ...
  private PurchaseRequestEntity buildPurchaseRequest(
      int familyDbId, int childId, ShopItemEntity item, String note) {
    return PurchaseRequestEntity.builder()
        .familyId(familyDbId)
        .childId(childId)
        .externalId(historyFactory.nextExternalId())
  ...
  private void publish(
      ApplicationOutboxEventType type, PurchaseRequestEntity request, int delta, Integer balance) {
    eventSupport.publish(
        type,
        request.getFamilyId(),
        request.getChildId(),
        request.getId(),
