...
public class WebPushService {
  private static final Set<ApplicationOutboxEventType> PARENT_EVENTS =
      Set.of(
          ApplicationOutboxEventType.TASK_REQUEST_CREATED,
          ApplicationOutboxEventType.REWARD_REQUEST_CREATED,
  ...
  public WebPushService(
      ...
      FamilyNotificationService preferences,
      SecurityAuditWriter audits,
      WebPushConfig config,
      WebPushProtocolAdapter protocol,
      ParentAccountRepository parentAccounts,
  ...
  public Optional<String> publicVapidKey() {
    ...
    if (config.vapidPrivateKey().filter(key -> !key.isBlank()).isEmpty()
        || config.vapidSubject().filter(key -> !key.isBlank()).isEmpty()) {
      return Optional.empty();
    }
  ...
  public void register(AuthContext auth, WebPushSubscriptionRequest request) {
    int familyId =
        families.getDbId(auth.familyId()).orElseThrow(() -> new IllegalArgumentException("family"));
    validate(request);
    String actorType = auth.isChild() ? "child" : "parent";
    Integer childId = auth.isChild() ? auth.childId() : null;
    if (auth.isChild() && childId == null) {
      throw new SecurityException("actor");
    }
    Integer parentId = auth.isChild() ? null : resolveParentAccountId(auth, familyId);
    if (!auth.isChild() && parentId == null) {
      throw new SecurityException("actor");
    }
    WebPushSubscriptionEntity entity =
        subscriptions.findByEndpoint(request.endpoint()).orElse(null);
    if (entity == null) {
      entity =
      ...
      entity.setParentAccountId(parentId);
      entity.setChildId(childId);
      entity.setP256dhKey(request.p256dh());
      entity.setAuthKey(request.auth());
  ...
  private Integer resolveParentAccountId(AuthContext auth, int familyDbId) {
    return Optional.ofNullable(auth.parentAccountId())
        ...
        .orElseGet(
            () ->
                Optional.ofNullable(auth.email())
                    .filter(email -> !email.isBlank())
                    .flatMap(parentAccounts::findByEmail)
                    .flatMap(
                        parent -> memberships.findByParentAndFamily(parent.getId(), familyDbId)))
  ...
  public void unregister(AuthContext auth, WebPushSubscriptionRequest request) {
    ...
    validate(request);
    Integer parentId = auth.isChild() ? null : resolveParentAccountId(auth, familyId);
    subscriptions.deleteForActor(
        request.endpoint(),
        familyId,
        auth.isChild() ? "child" : "parent",
        parentId,
        auth.isChild() ? auth.childId() : null);
  ...
  public int planDueEvents(Instant now, ApplicationOutboxEventRepository events) {
    int planned = 0;
    for (ApplicationOutboxEventEntity event :
        events.findPlanningCandidates(now.minus(Duration.ofMinutes(2)))) {
      ...
      List<WebPushSubscriptionEntity> recipients =
          parent
              ? subscriptions.findParents(event.getFamilyId())
  ...
  public void send(WebPushSubscriptionEntity subscription, WebPushDeliveryEntity delivery)
      throws Exception {
  ...
  private String preferenceKey(ApplicationOutboxEventType type, boolean parent) {
    if (parent) {
      return type == ApplicationOutboxEventType.REQUEST_RESOLVED
          ? "parentInviteAccepted"
          : "taskMarkedDone";
    }
    return switch (type) {
      case TASK_APPROVED -> "taskApproved";
      case TASK_REJECTED -> "taskRejected";
      case REWARD_APPROVED -> "rewardApproved";
      case REWARD_REJECTED -> "rewardRejected";
      case REWARD_PURCHASED -> "rewardAvailable";
      default -> "newTasks";
    };
  }
  ...
  private String payload(WebPushDeliveryEntity delivery) {
    return "{\"title\":\""
        + escape(delivery.getTitle())
        + "\",\"body\":\""
        + escape(delivery.getBody())
        + "\",\"url\":\""
        + escape(delivery.getDeepLink())
        + "\"}";
  }
