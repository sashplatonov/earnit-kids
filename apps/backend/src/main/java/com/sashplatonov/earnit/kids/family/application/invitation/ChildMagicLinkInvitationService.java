...
public class ChildMagicLinkInvitationService {
  ...
  public OperationResult<String> issue(String familyId, int childId) {
    ...
    if (family == null) {
      return failure("CHILD_NOT_FOUND");
    }
    var child = childRepository.findByIdOptional(childId).orElse(null);
    if (child == null
        || !family.getId().equals(child.getFamilyDbId())
        || !ChildStatus.ACTIVE.name().equals(child.getStatus())) {
      return failure("CHILD_NOT_FOUND");
    }
    invitationRepository.revokePending(family.getId(), childId, Instant.now());
    String token = tokenGenerator.generateHexToken(32);
    invitationRepository.persist(
        ChildMagicLinkInvitationEntity.builder()
  ...
  public OperationResult<Void> revoke(String familyId, int childId) {
    var family = familyRepository.findById(familyId).orElse(null);
    if (family == null
        || invitationRepository.revoke(family.getId(), childId, Instant.now()) == 0) {
  ...
  public OperationResult<List<Status>> status(String familyId, int childId) {
    ...
    if (family == null
        || childRepository
            .findByIdOptional(childId)
            .filter(child -> family.getId().equals(child.getFamilyDbId()))
            .isEmpty()) {
      return failure("CHILD_NOT_FOUND");
    }
    return OperationResult.success(
        invitationRepository.findByChild(family.getId(), childId).stream()
            .map(
                invitation ->
                    new Status(
                        invitation.getId(),
  ...
  public OperationResult<AuthPayload> consume(String token) {
    ...
    if (invitation == null
        || invitation.getStatus() != ChildMagicLinkInvitationEntity.Status.pending
        || invitation.getExpiresAt().isBefore(Instant.now())) {
      return failure("CHILD_INVITATION_INVALID");
    }
    if (invitationRepository.consume(invitation.getId(), Instant.now()) != 1) {
      return failure("CHILD_INVITATION_INVALID");
    }
  ...
  private String digest(String token) {
    try {
      ...
      mac.init(
          new SecretKeySpec(jwtConfig.secret().getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
