...
public class TelegramQuickActionServiceImpl implements TelegramQuickActionService {
  ...
  @Inject
  public TelegramQuickActionServiceImpl(
  ...
  public Optional<TelegramQuickActionResponse> load(long telegramUserId, Integer selectedChildId) {
    return identitiesFor(telegramUserId)
        .flatMap(
            identity -> {
              return familyId(identity.familyId())
                  .flatMap(
                      familyId -> {
                        if (!isParent(identity)) {
                          return Optional.ofNullable(identity.childId())
                              .flatMap(
                                  childId ->
  ...
  @Override
  public OperationResult<TelegramQuickActionResponse> requestTask(
      long telegramUserId, int childId, long taskId) {
    return mutate(
        telegramUserId,
        childId,
        false,
  ...
  @Override
  public OperationResult<TelegramQuickActionResponse> requestReward(
      long telegramUserId, int childId, long rewardId) {
    return mutate(
        telegramUserId,
        childId,
        false,
  ...
  @Override
  public OperationResult<TelegramQuickActionResponse> approveRequest(
      long telegramUserId, int childId, long requestId) {
    return mutate(
        telegramUserId,
        childId,
        true,
  ...
  @Override
  public OperationResult<TelegramQuickActionResponse> rejectRequest(
      long telegramUserId, int childId, long requestId) {
    return mutate(
        telegramUserId,
        childId,
  ...
  @Override
  public OperationResult<TelegramQuickActionResponse> adjustBalance(
      long telegramUserId, int childId, int amount) {
    return mutate(
        telegramUserId,
        childId,
        true,
        familyId ->
            actions.get().adjustBalance(familyId, childId, amount, "Telegram quick action"));
  }

  private OperationResult<TelegramQuickActionResponse> mutate(
      long telegramUserId, int childId, boolean parentOnly, Action action) {
    Optional<TelegramIdentityService.TelegramIdentity> identity =
        identitiesFor(telegramUserId)
            .filter(
                value ->
                    parentOnly
                        ? isParent(value)
                        : "child".equals(value.role())
                            && Integer.valueOf(childId).equals(value.childId()));
    if (identity.isEmpty()) {
      return OperationResult.failure(
          "TELEGRAM_SCOPE", "Telegram identity cannot perform this action");
    }
  ...
  private Optional<FamilyDataResponse> familyData(
      String familyId, Integer childId, boolean parent) {
    OperationResult<FamilyDataResponse> result =
        familyService.get().loadFamilyData(familyId, childId, parent);
  ...
  private Integer resolveChildId(Integer selectedChildId, FamilyDataResponse data) {
    if (selectedChildId != null
        && data.children().stream().anyMatch(child -> child.id() == selectedChildId)) {
      return selectedChildId;
    }
    if (data.lastSelectedChildId() != null
        && data.children().stream().anyMatch(child -> child.id() == data.lastSelectedChildId())) {
