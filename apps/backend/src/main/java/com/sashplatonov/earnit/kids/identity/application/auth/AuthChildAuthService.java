...
class AuthChildAuthService {
  ...
  OperationResult<AuthPayload> authenticateChild(String childToken) {
    if (childInvitationService != null) {
      return childInvitationService.consume(childToken);
    }
    if (childToken == null || childToken.isBlank()) {
      return OperationResult.failure("tokenMissing");
    }
    var child = legacyChildRepository.findByToken(childToken).orElse(null);
    if (child == null) {
      return OperationResult.failure("invalidLink");
    }
