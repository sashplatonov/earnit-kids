...
public class FamilySocialResource extends FamilyResourceSupport {
  ...
  public Response searchUser(
      ...
      @Parameter(description = "Nickname prefix to search for") @QueryParam("nickname")
          String nickname) {
  ...
  public Response addFriend(
      @Context ContainerRequestContext ctx,
      @RequestBody(required = true, description = "Friend relationship payload") @Valid
    ...
    return toVoidResponse(
        familyService.addFriend(auth.familyId(), auth.childId(), request.friendId()));
  }
