...
public class TelegramBotApiClient {
  ...
  public Long sendMessage(long chatId, String text, List<InlineButton> buttons) throws Exception {
    com.fasterxml.jackson.databind.JsonNode result =
        call("sendMessage", messagePayload(chatId, null, text, buttons));
    return result.path("result").path("message_id").isNumber()
        ? result.path("result").path("message_id").longValue()
        : null;
  }

  public void editMessageText(long chatId, long messageId, String text, List<InlineButton> buttons)
      throws Exception {
  ...
  public Long sendMessageWithReplyKeyboard(
      long chatId, String text, TelegramReplyKeyboard replyKeyboard) throws Exception {
    com.fasterxml.jackson.databind.JsonNode result =
        call("sendMessage", replyKeyboardPayload(chatId, null, text, replyKeyboard));
    return result.path("result").path("message_id").isNumber()
        ? result.path("result").path("message_id").longValue()
        : null;
  }
  ...
  public void removeReplyKeyboard(long chatId) throws Exception {
    call(
        "sendMessage",
        Map.of("chat_id", chatId, "text", "🔄", "reply_markup", Map.of("remove_keyboard", true)));
  }
  ...
  private com.fasterxml.jackson.databind.JsonNode call(String method, Object payload)
      throws Exception {
  ...
  private Map<String, Object> messagePayload(
      long chatId, Long messageId, String text, List<InlineButton> buttons) {
  ...
  private Map<String, Object> replyKeyboardPayload(
      long chatId, Long messageId, String text, TelegramReplyKeyboard replyKeyboard) {
    Map<String, Object> payload = new java.util.HashMap<>();
    payload.put("chat_id", chatId);
    if (messageId != null) {
      payload.put("message_id", messageId);
  ...
  private Map<String, Object> replyKeyboardPayload(TelegramReplyKeyboard replyKeyboard) {
    List<List<Map<String, Object>>> keyboardRows =
        replyKeyboard.rows().stream()
            .map(row -> row.buttons().stream().map(this::replyKeyboardButtonPayload).toList())
            .toList();
    return Map.of(
  ...
  public record InlineButton(
      String text, String url, String callbackData, String rowId, String urlKind) {
