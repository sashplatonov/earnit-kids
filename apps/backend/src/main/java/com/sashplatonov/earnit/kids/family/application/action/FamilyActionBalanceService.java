...
final class FamilyActionBalanceService {
  ...
  OperationResult<FamilyDataResponse> purchaseItem(String familyId, int childId, long itemId) {
    ...
    if (rewardLimit > 0
        && supportService.dailyRewardSpend(
                    childId, supportService.startOfFamilyDay(familyDbId, historyFactory.now()))
                + item.get().getPrice()
            > rewardLimit) {
    ...
    publish(
        ApplicationOutboxEventType.REWARD_PURCHASED,
        familyDbId,
        childId,
        null,
        -item.get().getPrice(),
  ...
  OperationResult<FamilyDataResponse> deleteHistoryEntry(
    ...
    Optional<HistoryEntryEntity> historyEntry =
        supportService.findHistoryEntry(familyDbId, childId, historyEntryId);
    ...
    historyRepository.persist(
        historyFactory.buildReversalHistory(familyDbId, childId, historyEntry.get()));
    return supportService.loadFamilyData(familyId, childId, true);
  }

  OperationResult<FamilyDataResponse> adjustBalance(
    ...
    historyRepository.persist(
        historyFactory.buildAdjustmentHistory(familyDbId, childId, amount, description));
    publish(
  ...
  private void publish(
      ApplicationOutboxEventType type,
      int familyId,
      int childId,
      Long requestId,
      int delta,
    ...
    if (eventPublisher != null) {
      eventPublisher.publish(
          type, familyId, childId, requestId, delta, balance, historyFactory.now());
    }
  }
}
