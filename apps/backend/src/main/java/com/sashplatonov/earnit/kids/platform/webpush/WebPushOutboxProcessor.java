...
public class WebPushOutboxProcessor {
    ...
    @Inject
    public WebPushOutboxProcessor(WebPushService service, WebPushDeliveryRepository deliveries,
                                  WebPushSubscriptionRepository subscriptions, ApplicationOutboxEventRepository events,
                                  WebPushConfig config) {
        this.service = service; this.deliveries = deliveries; this.subscriptions = subscriptions;
        this.events = events; this.config = config;
    }

    @Scheduled(every = "10s", concurrentExecution = Scheduled.ConcurrentExecution.SKIP)
    void scheduled() { if (config.enabled()) process(Instant.now()); }
    ...
    public int process(Instant now) {
        ...
        for (WebPushDeliveryEntity delivery : deliveries.findDue(now, now.minusSeconds(60))) {
            ...
            if (subscription == null) { terminal(delivery, "FAILED", now); continue; }
            try {
                service.send(subscription, delivery);
                terminal(delivery, "SENT", now); sent++;
                ...
                if (failure.status() == 404 || failure.status() == 410) {
                    subscriptions.deleteById(subscription.getId()); terminal(delivery, "FAILED", now);
                } else if (delivery.getAttemptCount() >= config.maxAttempts()) terminal(delivery, "FAILED", now);
                else retry(delivery, now);
            } catch (Exception failure) {
                delivery.setAttemptCount(delivery.getAttemptCount() + 1);
                delivery.setLastError(failure.getClass().getSimpleName());
                if (delivery.getAttemptCount() >= config.maxAttempts()) terminal(delivery, "FAILED", now);
                else retry(delivery, now);
            }
            ...
            if (event != null && events.allTransportsTerminal(event.getId())) {
                event.setPlanningCompletedAt(now); event.setPlanningStatus("COMPLETE");
            }
        }
        return sent;
    }
    private void retry(WebPushDeliveryEntity d, Instant now) { d.setClaimedAt(null); d.setNextAttemptAt(now.plusSeconds(1L << Math.min(6, d.getAttemptCount()))); }
    private void terminal(WebPushDeliveryEntity d, String status, Instant now) { d.setStatus(status); d.setTerminalAt(now); d.setClaimedAt(null); }
}
