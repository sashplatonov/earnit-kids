package com.sashplatonov.earnit.kids.family.infrastructure.persistence.child;
...
public class ChildRepository implements PanacheRepositoryBase<ChildEntity, Integer> {
  ...
  public List<ChildEntity> getActiveChildren(int familyDbId) {
    return recordQuery(
        ...
        () ->
            list(
                "familyDbId = ?1 AND status = ?2 ORDER BY id ASC",
                familyDbId,
                com.sashplatonov.earnit.kids.family.domain.model.child.ChildStatus.ACTIVE.name()),
        "familyDbId",
        String.valueOf(familyDbId));
  }
    ...
    return recordQuery(
        ...
        () ->
            list(
                "familyDbId = ?1 AND status = ?2 ORDER BY id ASC",
                familyDbId,
                com.sashplatonov.earnit.kids.family.domain.model.child.ChildStatus.INACTIVE.name()),
        "familyDbId",
        String.valueOf(familyDbId));
  }
  ...
  public Optional<ChildEntity> findByToken(String token) {
    return recordQuery(
        "child.findByToken", () -> find("token = ?1", token).firstResultOptional(), "token", token);
  }

  public Optional<ChildEntity> findByIdForUpdate(int childId) {
    return find("id = ?1", childId).withLock(LockModeType.PESSIMISTIC_WRITE).firstResultOptional();
  }
  ...
  public boolean updateSettings(
      int childId, String name, int dailyCoinLimit, int monthlyLimit, int dailyRewardLimit) {
    Optional<ChildEntity> opt = findByIdOptional(childId);
    if (opt.isEmpty()) {
      return false;
    }
  ...
  @Transactional
  public boolean updateGroupOrder(
      int childId,
    ...
    return recordQuery(
        ...
        () -> {
          if (excludeChildId != null) {
            return count(
                    "familyDbId = ?1 AND lower(name) = lower(?2) AND id != ?3",
                    familyDbId,
                    name,
                    excludeChildId)
                > 0;
  ...
  public List<ChildEntity> searchByNickname(String nameQuery, int excludeChildId) {
    return recordQuery(
        ...
        () ->
            find("lower(name) LIKE lower(?1) AND id != ?2", "%" + nameQuery + "%", excludeChildId)
                .page(0, 20)
                .list(),
        "excludeChildId",
        String.valueOf(excludeChildId),
  ...
  private <T> T recordQuery(
      String operation, java.util.function.Supplier<T> action, String... details) {
