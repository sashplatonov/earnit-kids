...
public class FamilyBalanceResource extends FamilyCommandResourceSupport {
  ...
  public Response adjustBalance(
      @Context ContainerRequestContext ctx, @RequestBody @Valid AdjustBalanceRequest request) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
    }
    OperationResult<FamilyDataResponse> result =
        familyActionService.adjustBalance(
