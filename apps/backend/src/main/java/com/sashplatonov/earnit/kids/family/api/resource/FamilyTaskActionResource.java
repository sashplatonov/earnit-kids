...
public class FamilyTaskActionResource extends FamilyCommandResourceSupport {
  ...
  @Inject
  public FamilyTaskActionResource(
      FamilyActionService familyActionService,
  ...
  public Response saveFamilyData(
      @Context ContainerRequestContext ctx, @RequestBody Map<String, Object> payload) {
  ...
  public Response completeTask(
    ...
    OperationResult<FamilyDataResponse> result =
        familyActionService.completeTask(auth.familyId(), childId, taskId);
    notifyDataUpdated(auth, childId, result);
    return toResponse(result);
  }
  ...
  public Response bulkTaskAction(
      @Context ContainerRequestContext ctx, @RequestBody @Valid BulkTaskActionRequest request) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
    }
    OperationResult<FamilyDataResponse> result =
        familyActionService.bulkTaskAction(auth.familyId(), request);
