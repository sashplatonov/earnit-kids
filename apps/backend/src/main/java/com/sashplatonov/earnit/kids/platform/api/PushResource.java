...
public class PushResource extends ResourceAuthSupport {
  ...
  public PushResource(WebPushService service) {
  ...
  public Response vapidPublicKey(@Context ContainerRequestContext ctx) {
    ...
    return service
        ...
        .map(
            key ->
                Response.ok(new WebPushPublicKeyResponse(key))
  ...
  @APIResponses({
    @APIResponse(
        responseCode = "200",
        description = "Push registration accepted",
  ...
  public Response register(
      WebPushSubscriptionRequest request, @Context ContainerRequestContext ctx) {
    Response authFailure = requireAuthResponse(ctx);
    if (authFailure != null) {
      return authFailure;
    }
