...
public class FamilyRequestActionResource extends FamilyCommandResourceSupport {
    @Inject
    public FamilyRequestActionResource(FamilyActionService action, FamilyService family, WebSocketNotificationService websocket,
                                       FamilyParentAccessService parentAccess) {
        super(action, family, websocket, parentAccess);
    }
    @POST @Path("/shop/{itemId}/request")
    @Operation(summary = "Create a child purchase request")
    public Response requestItemPurchase(@Context ContainerRequestContext ctx, @PathParam("itemId") long itemId,
                                        @QueryParam("childId") Integer childId, @Valid CreateRequestNoteRequest payload) {
        var auth = getAuthOrFail(ctx);
        if (auth == null || !auth.isChild() && !auth.canEditFamilyData()) return unauthorized();
        Integer effectiveChildId = auth.isChild() ? auth.childId() : childId;
        if (effectiveChildId == null) return badRequest(com.sashplatonov.earnit.kids.i18n.BackendMessages.message("errors.childIdRequired"));
    ...
    @POST @Path("/tasks/{taskId}/request")
    @Operation(summary = "Create a child task completion request")
    public Response requestTaskCompletion(@Context ContainerRequestContext ctx, @PathParam("taskId") long taskId,
                                          @QueryParam("childId") Integer childId, @Valid CreateRequestNoteRequest payload) {
        var auth = getAuthOrFail(ctx);
        if (auth == null || !auth.isChild() && !auth.canEditFamilyData()) return unauthorized();
        Integer effectiveChildId = auth.isChild() ? auth.childId() : childId;
    ...
    @POST @Path("/requests/{requestId}/approve")
    @Operation(summary = "Approve a child request")
    public Response approveRequest(@Context ContainerRequestContext ctx, @PathParam("requestId") long requestId,
                                   @QueryParam("childId") Integer childId) {
        return mutateRequest(ctx, childId, requestId, true);
    }
    @POST @Path("/requests/{requestId}/reject")
    @Operation(summary = "Reject a child request")
    public Response rejectRequest(@Context ContainerRequestContext ctx, @PathParam("requestId") long requestId,
                                  @QueryParam("childId") Integer childId) {
        return mutateRequest(ctx, childId, requestId, false);
    }
    private Response mutateRequest(ContainerRequestContext ctx, Integer childId, long requestId, boolean approve) {
        var auth = getAuthOrFail(ctx);
        if (auth == null || !auth.canEditFamilyData()) return unauthorized();
        OperationResult<FamilyDataResponse> result = approve
    ...
    @DELETE @Path("/requests/{requestId}")
    @Operation(summary = "Delete a request")
    public Response deleteRequest(@Context ContainerRequestContext ctx, @PathParam("requestId") long requestId,
                                  @QueryParam("childId") Integer childId) {
        var auth = getAuthOrFail(ctx);
        if (auth == null || (!auth.isAdmin() && !auth.isChild())) return unauthorized();
        Integer effectiveChildId = auth.isChild() ? auth.childId() : childId;
