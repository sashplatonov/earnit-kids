...
public class TelegramAccountConnectionServiceImpl implements TelegramAccountConnectionService {
  ...
  TelegramAccountConnectionServiceImpl(
    ...
    this.featureGate = runtime.featureGate();
  }

  @Override
  public OperationResult<TelegramAccountConnectionResponse> connection(
      String familyId, String email) {
    LOG.infof("Telegram connection check: familyId=%s, email=%s", familyId, email);
    return context(familyId, email)
        .map(
            context ->
        ...
        .orElseGet(
            () -> {
              LOG.warnf(
                  "Telegram connection check failed: context not found for familyId=%s, email=%s",
                  familyId, email);
  ...
  public OperationResult<TelegramLinkLaunchResponse> start(String familyId, String email) {
    ...
    challenges.persist(
        TelegramParentLinkChallengeEntity.builder()
    ...
    return OperationResult.success(
        new TelegramLinkLaunchResponse(botLaunchUrl(botUsername, token)));
  }
  ...
  public OperationResult<Void> complete(String token, String initData) {
    ...
    audits.persist(
        TelegramSecurityAuditEventEntity.builder()
            .familyId(family.getId())
  ...
  public OperationResult<Void> unlink(String familyId, String email) {
    ...
    var identity =
        identities.findActiveParentByParentAccountId(context.parentAccountId()).orElse(null);
  ...
  private boolean canComplete(
      TelegramParentLinkChallengeEntity challenge,
      FamilyEntity family,
