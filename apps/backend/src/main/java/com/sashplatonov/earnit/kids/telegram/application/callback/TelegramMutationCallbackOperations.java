...
import com.sashplatonov.earnit.kids.telegram.domain.model.TelegramCallbackActionEntity;
...
public final class TelegramMutationCallbackOperations {
  ...
  public TelegramMutationCallbackOperations(
  ...
  public TelegramIdentityService.TelegramCallbackToken create(
      Integer familyId,
      Integer identityId,
      String action,
      long targetId,
    ...
    TelegramCallbackActionEntity callback =
        TelegramCallbackActionEntity.builder()
            .familyId(familyId)
            .identityId(identityId)
            .action(action)
            .targetId(targetId)
            .secretDigest(TelegramTokenDigest.digest(token))
            .expiresAt(expiresAt)
            .createdAt(now)
            .build();
  ...
  public Optional<TelegramIdentityService.MutationCallback> consume(String token, Instant now) {
    Optional<TelegramCallbackActionEntity> found =
        callbacks.findByDigestForUpdate(TelegramTokenDigest.digest(token));
    if (found.isEmpty()) {
    ...
    return Optional.of(
        new TelegramIdentityService.MutationCallback(
            callback.getId(),
            callback.getFamilyId(),
            callback.getIdentityId(),
            callback.getAction(),
            callback.getTargetId()));
  }
