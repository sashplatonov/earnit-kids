...
@ApplicationScoped
public class WebPushSubscriptionRepository
    implements PanacheRepositoryBase<WebPushSubscriptionEntity, Long> {
  ...
  public void deleteForActor(
      String endpoint, int familyId, String actorType, Integer parentId, Integer childId) {
    delete(
        "endpoint = ?1 and familyId = ?2 and actorType = ?3 and parentAccountId = ?4 and childId = ?5",
        endpoint,
        familyId,
        actorType,
        parentId,
