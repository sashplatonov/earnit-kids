...
public class ParentInvitationService {
    ...
    @Transactional
    public OperationResult<ParentMembershipDto> create(String familyId, String email, String permission,
        ...
        if (parentAccountRepository.findByEmail(normalized)
            .flatMap(parent -> membershipRepository.findByParentAndFamily(parent.getId(), family.getId()))
            .isPresent()) {
        ...
        ParentEmailInvitationEntity invitation = ParentEmailInvitationEntity.builder()
            .familyId(family.getId()).normalizedEmail(normalized).permission(parsed.get())
            .tokenDigest(tokenHasher.digest(token, tokenHasher.activeKeyId()))
            .tokenDigestKeyId(tokenHasher.activeKeyId()).invitedByEmail(invitedByEmail)
            .expiresAt(Instant.now().plusSeconds(7 * 24 * 60 * 60)).build();
        invitationRepository.persist(invitation);
        try {
            emailSender.send(new ParentInvitationEmailSender.Email(normalized,
                publicInviteUrl(token), parsed.get().name(), invitation.getExpiresAt().toString()));
        } catch (RuntimeException ex) {
            invitationRepository.revoke(invitation.getId(), Instant.now());
            return failure("PARENT_INVITATION_DELIVERY_FAILED");
        }
        auditWriter.write(family.getId(), null, invitedByEmail, normalized,
            "parent_invitation_sent", "CREATED");
        return OperationResult.success(new ParentMembershipDto(null, normalized, null, null, null, null,
            parsed.get(), null, ParentEmailInvitationEntity.Status.pending.name()));
    }

    @Transactional
    public OperationResult<Void> revoke(String familyId, Integer invitationId, String actorEmail) {
        FamilyEntity family = familyRepository.findById(familyId).orElse(null);
        if (family == null || invitationRepository.revokeForFamily(invitationId, family.getId(), Instant.now()) == 0) {
            return failure("PARENT_INVITATION_NOT_FOUND");
        }
        auditWriter.write(family.getId(), null, actorEmail, null, "parent_invitation_revoked", "REVOKED");
    ...
    public OperationResult<Void> resend(String familyId, Integer invitationId, String actorEmail) {
        FamilyEntity family = familyRepository.findById(familyId).orElse(null);
        ParentEmailInvitationEntity invitation = family == null
            ? null : invitationRepository.findByIdOptional(invitationId).orElse(null);
        if (family == null || invitation == null || !family.getId().equals(invitation.getFamilyId())
            || invitation.getStatus() != ParentEmailInvitationEntity.Status.pending) {
            return failure("PARENT_INVITATION_NOT_FOUND");
        }
        String token = randomToken();
        Instant expiresAt = Instant.now().plusSeconds(7 * 24 * 60 * 60);
        if (invitationRepository.rotate(invitationId, family.getId(),
            tokenHasher.digest(token, tokenHasher.activeKeyId()), tokenHasher.activeKeyId(),
            expiresAt, Instant.now()) == 0) {
            return failure("PARENT_INVITATION_NOT_FOUND");
        }
        try {
            emailSender.send(new ParentInvitationEmailSender.Email(invitation.getNormalizedEmail(),
                publicInviteUrl(token), invitation.getPermission().name(), expiresAt.toString()));
        } catch (RuntimeException ex) {
            return failure("PARENT_INVITATION_DELIVERY_FAILED");
        }
        auditWriter.write(family.getId(), null, actorEmail, invitation.getNormalizedEmail(),
            "parent_invitation_resent", "RESENT");
        return OperationResult.success(null);
    }

    @Transactional
    public OperationResult<Continuation> begin(String token, String browserBinding, String postLoginPath) {
        ParentEmailInvitationEntity invitation = findByToken(token);
        if (invitation == null || invitation.getStatus() != ParentEmailInvitationEntity.Status.pending
            || invitation.getExpiresAt().isBefore(Instant.now())) {
            return failure("PARENT_INVITATION_INVALID");
        }
        String path = postLoginPath != null && postLoginPath.startsWith("/invite/parent")
            ? postLoginPath : "/invite/parent";
        String nonce = randomToken();
        OAuthInvitationContinuationEntity continuation = OAuthInvitationContinuationEntity.builder()
            .invitationId(invitation.getId()).nonceDigest(digest(nonce))
            .browserBindingDigest(digest(browserBinding)).issuedAt(Instant.now())
            .expiresAt(Instant.now().plusSeconds(600)).postLoginPath(path).build();
    ...
    public boolean consumeOAuth(Integer continuationId, String browserBinding, String email) {
        return continuationRepository.consume(continuationId, digest(browserBinding), Instant.now(), normalize(email)) == 1;
    }

    @Transactional
    public OperationResult<Void> accept(Integer continuationId, String browserBinding, String authenticatedEmail) {
        OAuthInvitationContinuationEntity continuation = continuationRepository
            .findConsumed(continuationId, digest(browserBinding), Instant.now()).orElse(null);
        if (continuation == null || !normalize(authenticatedEmail).equals(continuation.getVerifiedEmail())) {
            return failure("PARENT_INVITATION_NOT_AUTHORIZED");
        }
        ParentEmailInvitationEntity invitation = invitationRepository.findByIdOptional(continuation.getInvitationId()).orElse(null);
        if (invitation == null || !invitation.getNormalizedEmail().equals(continuation.getVerifiedEmail())) {
            return failure("PARENT_INVITATION_NOT_AUTHORIZED");
        }
        ...
        if (membershipRepository.findByParentAndFamily(parent.getId(), invitation.getFamilyId()).isEmpty()) {
            membershipRepository.persist(FamilyParentMembershipEntity.builder()
                .parentAccountId(parent.getId()).familyId(invitation.getFamilyId())
                .permission(invitation.getPermission()).status(com.sashplatonov.earnit.kids.family.domain.model.membership.MembershipStatus.active)
                .invitedByEmail(invitation.getInvitedByEmail()).invitedAt(Instant.now()).build());
        }
        auditWriter.write(invitation.getFamilyId(), parent.getId(), continuation.getVerifiedEmail(),
            continuation.getVerifiedEmail(), "parent_invitation_accepted", "ACCEPTED");
        return OperationResult.success(null);
    }
    ...
    private Optional<FamilyParentMembershipEntity.Permission> parse(String permission) {
        try { return Optional.of(FamilyParentMembershipEntity.Permission.valueOf(permission)); }
    ...
    private ParentEmailInvitationEntity findByToken(String token) {
        for (String keyId : tokenHasher.verificationKeyIds()) {
            Optional<ParentEmailInvitationEntity> invitation = invitationRepository.findByDigest(
                tokenHasher.digest(token, keyId), keyId);
    ...
    private String publicInviteUrl(String token) {
        return System.getenv().getOrDefault("APP_URL", "http://localhost:3000") + "/invite/parent/" + token;
    }
}
