...
public class AdminCoinEconomyRepository {
  ...
  public AdminCoinEconomyResponse.CoinMetrics getCoinMetrics(
      Instant periodStart, int activeChildrenInPeriod) {
  ...
  public AdminCoinEconomyResponse.BalanceMetrics getBalanceMetrics() {
    ...
    int n = sorted.size();
    double median =
        n % 2 == 0 ? (sorted.get(n / 2 - 1) + sorted.get(n / 2)) / 2.0 : sorted.get(n / 2);
  ...
  private double calcMedianTimeToFirstReward() {
    ...
    for (Object[] row : rows) {
      ...
      if (rewardAt != null && familyCreatedAt != null && rewardAt.isAfter(familyCreatedAt)) {
        days.add(
            ChronoUnit.DAYS.between(familyCreatedAt, rewardAt)
                + (double) (ChronoUnit.HOURS.between(familyCreatedAt, rewardAt) % 24) / 24.0);
      }
    }
    ...
    int n = days.size();
    return n % 2 == 0 ? (days.get(n / 2 - 1) + days.get(n / 2)) / 2.0 : days.get(n / 2);
  }
  ...
  public AdminCoinEconomyResponse.RewardMetrics getRewardMetrics(Instant periodStart) {
    String familiesWithRewardSql =
        """
            ...
            """;
    Long familiesWithReward =
        entityManager
            .createQuery(familiesWithRewardSql, Long.class)
