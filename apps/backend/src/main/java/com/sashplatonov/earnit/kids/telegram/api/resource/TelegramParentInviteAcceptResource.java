...
public class TelegramParentInviteAcceptResource {
  ...
  @Inject
  public TelegramParentInviteAcceptResource(
  ...
  public Response accept(@Valid ParentInviteAcceptRequest request) {
    ...
    return switch (invitations.accept(
        request.token(), request.initData(), null, timeProvider.now())) {
      case OperationResult.Success<TelegramIdentityService.TelegramIdentity> success -> {
        var identity = success.value();
        String familyId =
            families
                .get()
                .findFamilyIdByDbId(identity.familyId())
                .orElse("family-" + identity.familyId());
        Response.ResponseBuilder response = Response.ok(AuthResponse.success("admin", familyId));
        var cookies =
            identity.parentAccountId() == null
                ? cookieBuilder.buildAuthCookies(
                    request.legacyEmail(), "admin", familyId, null, "editor")
                : cookieBuilder.buildAuthCookies(
                    null, "admin", familyId, null, "editor", identity.parentAccountId());
