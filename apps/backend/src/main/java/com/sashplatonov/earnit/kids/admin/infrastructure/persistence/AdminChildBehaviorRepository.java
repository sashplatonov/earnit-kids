...
public class AdminChildBehaviorRepository {
  ...
  public AdminChildBehaviorResponse.ChildBehaviorMetrics getChildBehaviorMetrics(
      Instant periodStart) {
    ...
    double percentChildrenEarningNotSpending =
        totalActiveChildren > 0 ? (childrenEarningNotSpending * 100.0) / totalActiveChildren : 0.0;

    int childrenRequestedNotReceived = countChildrenRequestedNotReceived(periodStart);
    double percentChildrenRequestedNotReceived =
        totalActiveChildren > 0
            ? (childrenRequestedNotReceived * 100.0) / totalActiveChildren
            : 0.0;
    ...
    return AdminChildBehaviorResponse.ChildBehaviorMetrics.builder()
        ...
        .percentChildrenEarningNotSpending(
            Math.round(percentChildrenEarningNotSpending * 100.0) / 100.0)
        .childrenRequestedNotReceived(childrenRequestedNotReceived)
        .percentChildrenRequestedNotReceived(
            Math.round(percentChildrenRequestedNotReceived * 100.0) / 100.0)
        .build();
  }
  ...
  private int countChildrenEarningNotSpending(Instant periodStart) {
    String earnedSql =
        """
            ...
            """;
    List<Integer> earnedChildIds =
        entityManager
            .createQuery(earnedSql, Integer.class)
            .setParameter("earnType", HistoryEntryType.earn)
            .setParameter("periodStart", periodStart)
            .getResultList();

    if (earnedChildIds.isEmpty()) {
      return 0;
    }
    ...
    String spentSql =
        """
            ...
            """;
    List<Integer> spentChildIds =
        entityManager
            .createQuery(spentSql, Integer.class)
            .setParameter("approved", PurchaseRequestStatus.approved)
            .setParameter("periodStart", periodStart)
            .getResultList();
  ...
  private int countChildrenRequestedNotReceived(Instant periodStart) {
    String requestedSql =
        """
            ...
            """;
    List<Integer> requestedChildIds =
        entityManager
            .createQuery(requestedSql, Integer.class)
            .setParameter("periodStart", periodStart)
            .getResultList();

    if (requestedChildIds.isEmpty()) {
      return 0;
    }
    ...
    String receivedSql =
        """
            ...
            WHERE pr.status = :approved AND pr.createdAt >= :periodStart
            """;
    List<Integer> receivedChildIds =
        entityManager
            .createQuery(receivedSql, Integer.class)
            .setParameter("approved", PurchaseRequestStatus.approved)
            .setParameter("periodStart", periodStart)
            .getResultList();
