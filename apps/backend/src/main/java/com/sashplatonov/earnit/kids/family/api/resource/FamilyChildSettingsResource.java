...
import com.sashplatonov.earnit.kids.family.api.request.UpdateGroupOrderRequest;
import com.sashplatonov.earnit.kids.family.api.request.UpdateNicknameRequest;
...
public class FamilyChildSettingsResource extends FamilyResourceSupport {
  @Inject
  public FamilyChildSettingsResource(
        ...
        content =
            @Content(
                schema =
                    @Schema(
                        implementation =
                            com.sashplatonov.earnit.kids.shared.api.response.SimpleResponse
                                .class))),
    @APIResponse(
        responseCode = "400",
  ...
  public Response updateNickname(
      ...
      @Parameter(required = true, description = "Child id to rename") @PathParam("childId")
          int childId,
      @RequestBody(required = true, description = "New child nickname payload") @Valid
    ...
    OperationResult<Void> result =
        familyService.updateNickname(auth.familyId(), childId, request.name());
  ...
  @APIResponses({
    ...
    @APIResponse(
        responseCode = "401",
        description = "Child authentication required",
        content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
  })
  public Response updateOwnNickname(
      @Context ContainerRequestContext ctx,
      @RequestBody(required = true, description = "New nickname for the current child") @Valid
  ...
  @APIResponses({
    @APIResponse(
        responseCode = "200",
        description = "Child settings updated",
        content =
            @Content(
                schema =
                    @Schema(
  ...
  public Response updateChildSettings(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to update") @PathParam("childId")
          int childId,
      @RequestBody(required = true, description = "Updated child settings payload") @Valid
          UpdateChildSettingsRequest request) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
    }
    OperationResult<Void> result =
        familyService.updateChildSettings(
            auth.familyId(),
            childId,
            request.name(),
            request.dailyCoinLimit(),
      ...
      content =
          @Content(
              schema =
                  @Schema(
                      implementation =
                          com.sashplatonov.earnit.kids.shared.api.response.SimpleResponse.class)))
  public Response updateChildSettingsPost(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to update") @PathParam("childId")
          int childId,
      @RequestBody(required = true, description = "Updated child settings payload") @Valid
  ...
  public Response updateChildTheme(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to update") @PathParam("childId")
          int childId,
      @RequestBody(required = true, description = "Theme selection payload") @Valid
          UpdateThemeRequest request) {
    var auth = getAuthOrFail(ctx);
    if (auth == null
        || (!auth.isAdmin()
            && (!auth.isChild() || !java.util.Objects.equals(auth.childId(), childId)))) {
      return unauthorized();
    }
    OperationResult<Void> result =
        familyService.updateChildTheme(auth.familyId(), childId, request.theme());
    notifyChildUpdated(auth.familyId(), childId, result);
    return toVoidResponse(result);
  }
  @POST
          ...
          @Content(
              schema =
                  @Schema(
                      implementation =
                          com.sashplatonov.earnit.kids.shared.api.response.SimpleResponse.class)))
  public Response updateChildThemePost(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to update") @PathParam("childId")
          int childId,
      @RequestBody(required = true, description = "Theme selection payload") @Valid
  ...
  @APIResponses({
    ...
    @APIResponse(
        responseCode = "400",
        description = "Group order update failed",
        content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    @APIResponse(
        responseCode = "401",
        description = "Authentication required",
  ...
  public Response updateChildGroupOrder(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to update") @PathParam("childId")
          int childId,
      @RequestBody(required = true, description = "Group order payload") @Valid
    ...
    OperationResult<Void> result =
        familyService.updateChildGroupOrder(
            auth.familyId(),
            childId,
            request.section(),
            request.groups(),
  ...
  @APIResponses({
    ...
    @APIResponse(
        responseCode = "400",
        description = "Child creation failed",
        content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    @APIResponse(
        responseCode = "401",
        description = "Admin authentication required",
  ...
  public Response createChild(
      @Context ContainerRequestContext ctx,
      @RequestBody(required = true, description = "New child profile payload") @Valid
  ...
  public Response deleteChild(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to delete") @PathParam("childId")
          int childId) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
  ...
  public Response deactivateChild(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to deactivate") @PathParam("childId")
          int childId) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
  ...
  public Response reactivateChild(
      @Context ContainerRequestContext ctx,
      @Parameter(required = true, description = "Child id to reactivate") @PathParam("childId")
          int childId) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
