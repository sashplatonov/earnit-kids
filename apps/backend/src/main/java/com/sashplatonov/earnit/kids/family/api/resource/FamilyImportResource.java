...
public class FamilyImportResource extends FamilyCommandResourceSupport {
  ...
  public Response importTasks(
      @Context ContainerRequestContext ctx, @Valid ImportTasksRequest request) {
  ...
  public Response importShopItems(
      @Context ContainerRequestContext ctx, @Valid ImportShopItemsRequest request) {
    var auth = getAuthOrFail(ctx);
    if (auth == null || !auth.canEditFamilyData()) {
      return unauthorized();
    }
