...
public final class BackendMessages {
    private static final Map<String, String> EN_MESSAGES = Map.ofEntries(
        ...
        Map.entry("auth.googleNotConfigured", "Google sign-in is not configured"),
        Map.entry("auth.googleEmailNotVerified", "Google account email must be verified"),
        Map.entry("auth.googleAccountNotLinked", "No family account is linked to this Google email yet"),
    ...
    private static final Map<String, String> RU_MESSAGES = Map.ofEntries(
        ...
        Map.entry("auth.googleNotConfigured", "Вход через Google не настроен"),
        Map.entry("auth.googleEmailNotVerified", "Email аккаунта Google должен быть подтвержден"),
        Map.entry("auth.googleAccountNotLinked", "С этим Google email пока не связана семейная учетная запись"),
