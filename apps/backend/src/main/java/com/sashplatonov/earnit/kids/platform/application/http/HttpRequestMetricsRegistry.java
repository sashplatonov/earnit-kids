...
public class HttpRequestMetricsRegistry {
  ...
  private static final String HTTP_RESPONSE_PAYLOAD_METRIC =
  ...
  public void record(String method, String path, int status, long durationMs, long payloadBytes) {
    ...
    endpoints
        .computeIfAbsent(
            safeMethod + " " + safePath, key -> new EndpointMetrics(safeMethod, safePath))
  ...
  public HttpMetricsResponse snapshot() {
    ...
    for (EndpointMetrics metrics : endpoints.values()) {
      ...
      double payloadMb =
          count == 0 ? 0 : Math.round((double) totalBytes / 1_048_576.0 * 100) / 100.0;
      topEndpoints.add(
          new HttpMetricsResponse.HttpEndpointMetrics(
              metrics.method,
              metrics.path,
              count,
              errors,
    ...
    topEndpoints.sort(
        Comparator.comparingLong(HttpMetricsResponse.HttpEndpointMetrics::count)
            .reversed()
  ...
  private void recordMicrometerMetrics(
      String method, String path, int status, long durationMs, long payloadBytes) {
