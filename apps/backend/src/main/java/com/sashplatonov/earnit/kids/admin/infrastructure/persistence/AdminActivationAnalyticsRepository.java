...
public class AdminActivationAnalyticsRepository {
  ...
  public List<AdminActivationFunnelResponse.FunnelStage> getActivationFunnel(Instant periodStart) {
    ...
    stages.add(
        createStage(
            "earned_coins", "Заработали монеты", earnedCoins, childCompletedTask, earnedCoins));
    stages.add(createStage("has_reward", "Есть награда", hasReward, earnedCoins, hasReward));
    stages.add(
        createStage(
            "received_reward", "Получили награду", receivedReward, hasReward, receivedReward));

    return stages;
  }

  private AdminActivationFunnelResponse.FunnelStage createStage(
      String key, String label, int count, int previousCount) {
  ...
  private AdminActivationFunnelResponse.FunnelStage createStage(
      String key, String label, int count, int previousCount, int initialCount) {
    double percentFromPrevious = previousCount > 0 ? (count * 100.0) / previousCount : 0.0;
