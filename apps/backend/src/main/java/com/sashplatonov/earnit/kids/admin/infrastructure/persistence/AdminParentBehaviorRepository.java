...
public class AdminParentBehaviorRepository {
  ...
  public AdminParentBehaviorResponse.ParentBehaviorMetrics getParentBehaviorMetrics(
      Instant periodStart) {
    ...
    return AdminParentBehaviorResponse.ParentBehaviorMetrics.builder()
        ...
        .familiesUsingCustomContentPercent(
            Math.round(familiesUsingCustomContentPercent * 100.0) / 100.0)
  ...
  private double calcFamiliesUsingCatalogPercent(Instant periodStart) {
    ...
    return totalActiveFamilies > 0
        ? (familiesUsingCatalog.doubleValue() / totalActiveFamilies) * 100.0
        : 0.0;
  }
  ...
  private double calcFamiliesUsingCustomContentPercent(Instant periodStart) {
    ...
    long totalActiveFamilies = countActiveFamilies(periodStart);
    return totalActiveFamilies > 0
        ? (familiesUsingCustom.doubleValue() / totalActiveFamilies) * 100.0
        : 0.0;
  }
  ...
  private double calcMedianApprovalDelayHours(Instant periodStart) {
    ...
    int n = delays.size();
    return (n % 2 == 0) ? (delays.get(n / 2 - 1) + delays.get(n / 2)) / 2.0 : delays.get(n / 2);
  }
