...
function getRequestHost(req) {
    if (req) {
        ...
        if (req.headers.host) return req.headers.host.split(':')[0].toLowerCase();
    }
...
function shouldIncludeClarityScript(req) {
    ...
    if (!CLARITY_PROJECT_ID || !/^[a-zA-Z0-9]+$/.test(CLARITY_PROJECT_ID)) return false;
    return !isLocalHost(getRequestHost(req));
}

function getClarityScript(req) {
    if (!shouldIncludeClarityScript(req)) return '';
...
function getBaseUrl(req) {
    if (req && req.headers.host) return `${getProtocol(req)}://${req.headers.host}`;
...
function buildSeoReplacements(req, seoData = {}) {
    ...
    if (!schemaData.url) schemaData.url = canonicalUrl;
    ...
    return {
        ...
        '{{SCHEMA_JSON}}': JSON.stringify(schemaData, null, 4)
    };
}
