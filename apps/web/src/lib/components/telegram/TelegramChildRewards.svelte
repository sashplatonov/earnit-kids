...
<section aria-labelledby="child-rewards-title"><div class="heading"><h2 id="child-rewards-title">Rewards</h2><span>Spend your coins</span></div>{#if !affordable.length && !nextGoal}<p class="empty">No rewards available right now.</p>{:else}<TelegramGroupedCatalog kind="reward" items={items} {pendingIds} on:request={(event) => { selected = event.detail; status = 'idle'; }} />{/if}{#if nextGoal}<div class="goal" aria-label="Next goal"><span class="entity-icon"><TelegramIcon name={getTelegramEntityIcon({ kind: 'reward', title: nextGoal.name, group: nextGoal.groupName })} size={20} label="Reward" /></span><div class="goal-text"><span class="goal-title">{stripLeadingEmoji(nextGoal.name)}</span><div class="goal-track" role="progressbar" aria-valuemin="0" aria-valuemax={nextGoal.price} aria-valuenow={$appStore.balance} aria-label={nextGoal.name}><span style={`width: ${goalPercent}%`}></span></div><span class="goal-meta"><TelegramCoin size={13} />{nextGoal.price} · {goalMissing} more to go</span></div></div>{/if}<TelegramActionStatus state={status} message={message} /></section>
