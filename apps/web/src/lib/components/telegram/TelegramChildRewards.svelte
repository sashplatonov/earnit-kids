...
<section aria-labelledby="child-rewards-title">
    <div class="heading">
        ...
        {#if nextGoal}<div class="goal-heading"><TelegramCoin size={13} /><span class="goal-heading-title">{stripLeadingEmoji(nextGoal.name)}</span><span class="goal-heading-progress">{$i18n.t('app.telegram.childRewards.progress', { current: $appStore.balance, target: nextGoal.price })}</span></div>{:else}<span>{$i18n.t('app.telegram.childRewards.spendYourCoins')}</span>{/if}
    </div>
