...
{#if open}
    ...
    <div class="sheet" role="dialog" aria-modal="true" aria-labelledby="parent-access-title" tabindex="-1">
        ...
        <ParentAccessPanel embedded compact />
...
<style>
    .sheet-backdrop{position:fixed;inset:0;z-index:40;background:rgb(15 24 45 / 35%)}.sheet{position:fixed;inset:auto 0 0;z-index:41;display:grid;gap:.65rem;padding:.75rem max(.75rem,env(safe-area-inset-left)) calc(.75rem + env(safe-area-inset-bottom));border-radius:1.1rem 1.1rem 0 0;background:#fff;box-shadow:0 -1rem 3rem rgb(27 39 73 / 18%);max-height:88dvh;overflow:auto}h2{margin:0;color:#18243d;font-size:1.15rem}.close{display:flex;align-items:center;justify-content:center;gap:.5rem;width:100%;min-height:2.75rem;padding:.5rem .7rem;border:0;border-radius:.7rem;background:#fff7f7!important;border:1px solid #f1c7ca!important;color:#a84a50!important;font:inherit;font-weight:700}
