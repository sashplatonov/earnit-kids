...
{#if open}
    ...
    <div class="sheet" role="dialog" aria-modal="true" aria-labelledby="parent-access-title" tabindex="-1">
        ...
        <section class="telegram-option" aria-labelledby="telegram-invite-title">
            ...
            {:else if !inviteLink}<label for="telegram-parent-name">{$i18n.t('app.telegram.parents.nameLabel')}</label><input id="telegram-parent-name" bind:value={parentName} autocomplete="name" /><button type="button" disabled={busy} on:click={createInvite}><TelegramIcon name="link" size={18} label={$i18n.t('app.telegram.parents.createLink')} />{busy ? '…' : $i18n.t('app.telegram.parents.createLink')}</button>
            {:else}<p role="status">{$i18n.t('app.telegram.parents.linkReady')}</p><button type="button" on:click={copy}><TelegramIcon name="copy" size={18} label={$i18n.t('app.telegram.parents.copyLink')} />{copied ? $i18n.t('app.telegram.parents.copied') : $i18n.t('app.telegram.parents.copyLink')}</button>{/if}
