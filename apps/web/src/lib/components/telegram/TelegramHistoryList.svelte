...
<section aria-label={$i18n.t('app.telegram.history.history')}>
    ...
    {:else}<TelegramListSurface label={$i18n.t('app.telegram.history.history')}>{#each entries as entry (entry.id)}<TelegramEntityRow>
        ...
        <time class="history-time" slot="metadata" datetime={entry.createdAt ?? undefined}>{entry.createdAt ? formatLastUsedTime(entry.createdAt, $i18n.locale) : $i18n.t('app.telegram.history.recently')}</time>
...
<style>
    ...
    :global(.history-title) { display:block; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#18243d; font-size:.8rem; font-weight:600; line-height:1.2; }
