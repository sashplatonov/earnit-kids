...
<section aria-label={$i18n.t('app.telegram.history.history')}>
    ...
    {:else}<TelegramListSurface label={$i18n.t('app.telegram.history.history')}>{#each entries as entry (entry.id)}<TelegramEntityRow>
        ...
        <div slot="metadata"><time class="history-time" datetime={entry.createdAt ?? undefined}>{entry.createdAt ? formatLastUsedTime(entry.createdAt, $i18n.locale) : $i18n.t('app.telegram.history.recently')}</time>{#if entry.groupName}<span class="history-group"><TelegramIcon name={getTelegramEntityIcon({ kind: 'category', group: entry.groupName })} size={13} label={entry.groupName} />{stripLeadingEmoji(entry.groupName)}</span>{/if}</div>
