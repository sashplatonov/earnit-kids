<script lang="ts">
    ...
    function historyIcon(entry: HistoryEntry) {
        // Group graphics take precedence in history, matching the group marker used by task lists.
        return getTelegramEntityIcon({ kind: historyKind(entry.type), title: entry.groupName ? null : historyTitle(entry), group: entry.groupName });
    }
...
<section aria-label={$i18n.t('app.telegram.history.history')}>
    ...
    {:else}<TelegramGroupSubnav groups={groups} selected={selectedGroup} kind="history" allLabel={$i18n.t('app.telegram.groupSubnav.all')} moreLabel={$i18n.t('app.telegram.groupSubnav.more')} allGroupsTitle={$i18n.t('app.telegram.groupSubnav.allGroups')} onSelect={(group) => selectedGroup = group} />
        ...
        <span slot="icon"><TelegramIcon name={historyIcon(entry)} size={20} label={$i18n.t('app.telegram.history.activity')} /></span>
