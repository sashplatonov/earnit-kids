...
<section aria-labelledby="child-tasks-title"><div class="heading"><h2 id="child-tasks-title">Tasks · Today</h2><span>{$appStore.tasks.length} available</span></div><TelegramGroupedCatalog kind="task" {items} {pendingIds} on:request={(event) => { selected = event.detail; status = 'idle'; }} /><TelegramActionStatus state={status} message={message} /></section>
