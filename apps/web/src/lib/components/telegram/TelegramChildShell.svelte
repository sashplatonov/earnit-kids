...
<style>
    .child-workspace { box-sizing:border-box; display:flex; flex-direction:column; width:100%; max-width:48rem; min-height:100vh; margin:0 auto; padding:calc(.75rem + env(safe-area-inset-top)) 1rem calc(2rem + env(safe-area-inset-bottom)); } .state { padding:2rem 1rem; color:#66718a; text-align:center; } button { min-height:2.75rem; margin-top:1rem; padding:.6rem .85rem; border:1px solid #3867d6; border-radius:.7rem; background:#3867d6; color:#fff; font:inherit; } @keyframes skeleton-pulse { 0%,100% { opacity:.55; } 50% { opacity:1; } } @media (prefers-reduced-motion: reduce) { .panel-skeleton .skeleton-line { animation:none; } } .tabs { display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:.45rem; flex-shrink:0; margin-bottom:.75rem; } .tabs button { margin-top:0; background:#fff; color:#33415f; } .tabs button.active { background:#fff; color:#2854ba; font-weight:750; } .tab-panel { flex:1 1 auto; min-height:0; } .panel-skeleton { display:flex; flex-direction:column; gap:.75rem; padding:1rem 0; } .panel-skeleton .skeleton-line { height:4.5rem; border-radius:.8rem; background:#e8eaf1; animation:skeleton-pulse 1.3s ease-in-out infinite; } .site-link { display:flex; justify-content:center; flex-shrink:0; margin-top:1.25rem; } .site-link a { display:inline-flex; align-items:center; gap:.3rem; color:#8a93a8; font-size:.78rem; text-decoration:none; } .site-link a:hover { color:#3867d6; } .site-link a:focus-visible { outline:3px solid #80aaff; outline-offset:2px; border-radius:.3rem; }
