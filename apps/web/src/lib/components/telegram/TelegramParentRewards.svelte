...
<section class="panel" aria-labelledby="rewards-title">
    <div class="section-heading"><div><p class="eyebrow">Manage catalog</p><h2 id="rewards-title">Rewards</h2></div>{#if canEdit}<button class="primary" type="button" on:click={add}>Add reward</button>{/if}</div>
    {#if !$appStore.shopItems.length}<p class="muted">No rewards for this child yet.</p>{:else}<div class="items">{#each $appStore.shopItems as item (item.id)}<article class:archived={item.isActive === false}><div><h3>{item.name}</h3><p>{item.price} 🪙 · {item.groupName || 'Ungrouped'}</p></div>{#if canEdit}<button type="button" aria-label={`Edit ${item.name}`} on:click={() => edit(item)}>Edit</button>{/if}</article>{/each}</div>{/if}
