...
export function createLiveCoinShopDemoActions(read: () => LiveCoinShopDemoSnapshot, commit: (next: LiveCoinShopDemoSnapshot) => LiveCoinShopDemoSnapshot): LiveCoinShopDemoActions {
    ...
    const decide = (requestId: string | number, status: 'approved' | 'rejected') => mutate((state) => {
        ...
        if (status === 'approved' && requestEntry.taskId) {
            ...
            return { ...state, app: { ...state.app, children: state.app.children.map((entry) => entry.id === target.id ? { ...entry, balance } : entry), balance: String(state.app.currentChildId) === String(target.id) ? balance : state.app.balance, requests: state.app.requests.map((entry) => entry.id === requestEntry.id ? { ...entry, status } : entry), history: [{ id: `live-demo-history-${state.app.history.length + 1}`, type: 'task_completed', amount: task.coins, title: task.name, taskId: task.id, childId: target.id, createdAt: NOW }, ...state.app.history] } };
        }
