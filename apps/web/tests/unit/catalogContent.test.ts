...
describe('catalog content validation', () => {
    ...
    it('every title begins with exactly one meaningful emoji', () => {
        for (const item of all) {
            const stripped = stripEmoji(item.title);
            expect(stripped, `title should keep text after emoji: ${item.title}`).not.toBe(item.title);
            expect(stripped.trim(), `title should not be blank: ${item.title}`).not.toBe('');
        }
    });

    it('every title is non-blank', () => {
        for (const item of all) {
            expect(item.title.trim(), `blank title for ${item.id}`).not.toBe('');
        }
    });

    it('task titles read as an action ("что мне нужно сделать")', () => {
        for (const task of baseData.catalog.tasks) {
            const text = stripEmoji(task.title).toLowerCase();
    ...
    it('reward titles read as a concrete result ("что я получу")', () => {
        for (const reward of baseData.catalog.rewards) {
            const text = stripEmoji(reward.title).toLowerCase();
    ...
    it('every item has a group and frequency metadata', () => {
        for (const item of all) {
            expect(item.groupName?.trim(), `groupName for ${item.id}`).not.toBe('');
