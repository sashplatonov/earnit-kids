...
function renderTasks(copy, locale) {
    return demoData.tasks.length ? element('ul', 'demo-list', demoData.tasks.map((task) => element('li', 'demo-row', [
        element('div', 'demo-row-main', [element('h3', null, [text(task.name[locale])]), label(copy.task.group, task.group), label(copy.task.repeat, task.repeat)]),
        element('strong', 'demo-amount demo-amount-positive', [text(`+${task.coins} ${copy.coins}`)]),
    ]))) : element('p', 'demo-empty', [text(copy.empty)]);
}

function renderRewards(copy, locale) {
    return demoData.rewards.length ? element('ul', 'demo-list', demoData.rewards.map((reward) => element('li', 'demo-row', [
        element('div', 'demo-row-main', [element('h3', null, [text(reward.name[locale])]), label(copy.reward.group, reward.group), label(copy.reward.available, reward.available ? 'Yes' : 'No')]),
...
function renderHistory(locale, copy) {
    return demoData.history.length ? element('ul', 'demo-list', demoData.history.map((entry) => element('li', 'demo-row', [
        element('div', 'demo-row-main', [element('h3', null, [text(entry.label[locale])]), element('time', 'demo-meta', [text(formatDemoDate(entry.date, locale))])]),
...
function renderRequests(locale, copy) {
    return demoData.requests.length ? element('ul', 'demo-list', demoData.requests.map((request) => element('li', 'demo-row', [
        element('div', 'demo-row-main', [element('h3', null, [text(request.label[locale])]), label(copy.request.submitted, request.kind === 'task' ? copy.request.task : copy.request.reward), element('time', 'demo-meta', [text(formatDemoDate(request.date, locale))])]),
...
export function renderDemo(documentRef = document, windowRef = window) {
    ...
    const summary = element('div', 'demo-summary', [element('div', null, [element('span', 'demo-eyebrow', [text(copy.sampleChild)]), element('strong', null, [text(locale === 'ru' ? 'Анна' : 'Anna')])]), element('div', null, [element('span', 'demo-eyebrow', [text(copy.balance)]), element('strong', null, [text(`${demoData.child.balance} ${copy.coins}`)])])]);
    const tabs = element('div', 'demo-tabs', []);
    const panels = element('div', 'demo-panels', []);
    const renderers = { tasks: () => renderTasks(copy, locale), rewards: () => renderRewards(copy, locale), history: () => renderHistory(locale, copy), requests: () => renderRequests(locale, copy) };
