...
function handleApprovePurchase(req) {
    ...
    if (item) {
        ...
        if (err && !confirm(`${err}. Все равно подтвердить?`)) return;
    }

    updateBalanceLocally(req.childId, -req.coins);
    addHistoryEntry({ type: 'spend', amount: req.coins, description: req.taskName || 'Покупка', relatedId: req.itemId || req.taskId, moneyAmount: req.moneyAmount || 0, childIdOverride: req.childId });
...
export function approveRequest(reqId) {
    ...
    const desc = src ? [src.name, src.group ? `Гр: ${src.group}` : '', src.comment ? `Оп: ${src.comment}` : ''].filter(Boolean).join(' | ') : req.taskName;

    addHistoryEntry({ type: 'earn', amount: req.coins, description: desc, relatedId: req.taskId, childIdOverride: req.childId });
