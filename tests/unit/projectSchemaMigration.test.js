...
test('project schema migration handles duplicate restored tables', () => {
    assert.match(migrationSql, /to_regclass\(format\('public\.\%I', app_table_name\)\) IS NOT NULL/);
    assert.match(migrationSql, /to_regclass\(format\('__DB_SCHEMA__\.\%I', app_table_name\)\) IS NOT NULL/);
...
test('project schema migration replaces duplicate trigger function before move', () => {
    assert.match(migrationSql, /n\.oid = '__DB_SCHEMA__'::regnamespace/);
